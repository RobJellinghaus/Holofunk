...
/* HOLOFUNK HACK: REMOVE COMMENT BRACKETS
#ifdef _WIN32
*/
...
static void mainInit()
{
	...
	s_bassfunc = (BASS_FUNCTIONS*)GetProcAddress(GetModuleHandle((LPWSTR)L"bass"), "_");
	bassversion = BASS_GetVersion();
	if (!s_bassfunc || (bassversion != MAKELONG(2, 2) && HIWORD(bassversion) != 0x0203 && HIWORD(bassversion) != 0x0204))
	{
		MessageBox(0, (LPWSTR)L"Incorrect BASS.DLL version (2.2 - 2.4 is required)", (LPWSTR)L"BASS_VST" BASS_VST_VERSION_STR, MB_ICONERROR);
		return;
	}
						                 ...
						                 VstIntPtr value, void *ptr, float opt)
{
	...
	switch( opcode )
	{
		...
		case __audioMasterWantMidiDeprecated:
			OutputDebugString((LPWSTR)"plug called audioMasterWantMidi\n");
			break;
...
static BASS_VST_PLUGIN* refHandle_checkProgramIndex(DWORD vstHandle, int programIndex)
{
	...
	if( programIndex < -1 // Holofunk change: Sugar Bytes says Turnado wants to and can deal with such an index for bank swapping 
...
BOOL BASS_VSTDEF(BASS_VST_SetProgram)(DWORD vstHandle, int programIndex)
{
	...
	enterVstCritical(this_);
		...
		currentProgramIndex = this_->aeffect->dispatcher(this_->aeffect, effGetProgram, 0, 0, NULL, 0.0);

		bool newIndexDiffers = programIndex != currentProgramIndex;
