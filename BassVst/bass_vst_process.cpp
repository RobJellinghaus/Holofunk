...
static bool allocChanBuffers(BASS_VST_PLUGIN* this_, long numInputs, long numOutputs, long numBytes)
{
	 ...
	 || this_->buffersOut[numOutputs-1] == NULL )
	{
		...
		// for x64 allocate 16-byte aligned
		const size_t x64alignment = (size_t)16;
		int i;
		for( i = 0; i < numInputs; i++ )
		{ 
			if ((this_->buffersIn[i] = (float*)_aligned_malloc(numBytes*BUFFER_HEADROOM_MULT, x64alignment)) == NULL)
			{
				freeChansBuffers(this_);
				return false;
			}
		...
		{
			if ((this_->buffersOut[i] = (float*)_aligned_malloc(numBytes*BUFFER_HEADROOM_MULT, x64alignment)) == NULL)
			{
				freeChansBuffers(this_);
				return false;
			}
