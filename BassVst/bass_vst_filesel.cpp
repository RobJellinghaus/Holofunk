...
long fileSelOpen(BASS_VST_PLUGIN* this_, VstFileSelect* vstFs)
{
	...
	ofn.lpstrFilter = (LPWSTR)"*.*\0*.*\0\0";
	ofn.lpstrFile = (LPWSTR)vstFs->reserved;
	ofn.lpstrFile[0] = 0;
	ofn.nMaxFile = BUFFER_BYTES-1;
	ofn.lpstrTitle = (LPWSTR)vstFs->title;
	...
	if( vstFs->command == kVstMultipleFilesLoad )
	{
		...
		vstFs->returnMultiplePaths[0] = (char*)ofn.lpstrFile;
	...
	{	
		...
		if( vstFs->command == kVstDirectorySelect )
		{
			...
			char* ptr = (char*)strrchr((const char*)ofn.lpstrFile, '\\');
			if( ptr )
			{
				ptr++;
				*ptr = 0;
		...
		if( vstFs->returnPath )
		{
			...
			if( strlen((char*)ofn.lpstrFile) < 1000 )
				strcpy(vstFs->returnPath, (char*)ofn.lpstrFile);
			fileSelClose(this_, vstFs);
		}
		else
		{
			// buffer is allocated by us - fileSelClose() must be called by the user
			vstFs->returnPath = (char*)ofn.lpstrFile;
		}
	}
